...
import Environment, { extendEnvironment ,createEnvironment} from '../environment';
...
function createConstructor(body: FNode, parameters: FNode, environment: Environment, lineConsole: LineConsole) {
    return function* name(argumentsArray: any[]) {
        ...
        paramEnvironment.setItem("___THIS___", createEnvironment());
...
export default function* classDefProcessorFunction(tree: FNode, environment: Environment, lineConsole: LineConsole):processorType {
    ...
    outerEnvironment.setItem(identifierRef.leafText, createConstructor(body, paramter, outerEnvironment, lineConsole));
}
